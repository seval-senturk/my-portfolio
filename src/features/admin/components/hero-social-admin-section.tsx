...
import { saveHeroSocialLinksAction } from "@/features/admin/actions/content.actions";
...
interface HeroSocialAdminSectionProps {
  initialLinks: Array<{
    platform: string;
    label: string;
    href: string;
...
export function HeroSocialAdminSection({
  initialLinks,
}: HeroSocialAdminSectionProps) {
  ...
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const result = await saveHeroSocialLinksAction(formData);
      setStatus(
        result.success
  ...
  return (
    <form action={handleSubmit} className="space-y-4">
      <AdminFormSection
        title={adminTr.hero.sections.social}
        description={adminTr.hero.sections.socialDesc}
      >
            ...
            >
              <label className="flex flex-col gap-1 text-small">
                <span className="font-medium text-foreground">
                  {adminTr.hero.fields.socialPlatform}
                </span>
                <select
              ...
              <AdminTextField
                ...
                label={adminTr.hero.fields.socialLabel}
                defaultValue={row.label}
              />
              <AdminTextField
                id={`social_${index}_href`}
                name={`social_${index}_href`}
                label={adminTr.hero.fields.socialHref}
              ...
              <label className="flex items-end gap-2 pb-2 text-small">
                ...
                />
                <span>{adminTr.hero.fields.socialVisible}</span>
              </label>
            </div>
          ))}
        </div>
